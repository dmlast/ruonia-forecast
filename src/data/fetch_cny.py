...
def get_cny_rub(
    ...
    df = df.rename(columns={"data": "DATE", "nominal": "nominal", "curs": "curs"})
    ...
    return df[["DATE", "cny_rub", "PUBLICATION_TS", "EFFECTIVE_DATE"]]
...
def cli(
    ...
    out = RAW_DIR / f"cny_rub_{start_date:%Y%m%d}_{end_date:%Y%m%d}.parquet"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Fetch official CNY/RUB history via CBR Excel endpoint"
    )
    ...
    df = get_cny_rub(args.start, args.end)
    out_path = Path(args.out) if args.out else RAW_DIR / f"cny_rub_{args.start:%Y%m%d}_{args.end:%Y%m%d}.parquet"
